...
router.get('/user/:username/:type/:nanoid([A-Za-z0-9_\\-]{21})', async (req, res) => {
...
router.get('/user/:username/:type/:nanoid([A-Za-z0-9_\\-]{21})/:collection', async (req, res) => {
  const { objectStorage, formatter, authorizer } = req.app.locals
...
router.get('/user/:username/:type/:nanoid([A-Za-z0-9_\\-]{21})/:collection/:n(\\d+)', async (req, res) => {
  const { objectStorage, formatter, authorizer } = req.app.locals
  if (!['replies', 'likes', 'shares'].includes(req.params.collection)) {
    throw createHttpError(404, 'Not Found')
  }
