...
export class ObjectStorage {
    ...
    static async initialize (connection) {
    ...
    static async getCollection (id, property) {
        ...
        assert.ok(id, 'id is required')
        assert.ok(property, 'property is required')
        let [totalItems, first, createdAt] = await this.#collectionInfo(id, property)
    ...
    static async addToCollection (id, property, object) {
        ...
        let count = await this.#itemCount(id, property, first)

        let page = (count >= this.#MAX_ITEMS_PER_PAGE) ? first + 1 : first
    ...
    static async #itemCount (id, property, page) {
        assert.ok(this.#connection, 'ObjectStorage not initialized')
        assert.ok(id, 'id is required')
        assert.equal(typeof id, 'string', 'id must be a string')
        assert.ok(property, 'property is required')
    ...
    static async removeFromCollection (id, property, object) {
